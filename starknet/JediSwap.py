...
from starknet.constants import JEDISWAP_ADDR, ABI_TOKENS, ABI_JEDISWAP
...
class JediSwap(Swap):
    ...
    def __init__(self, 
        ...
        super().__init__(account, logger, gwei_checker, ErrorRetry, ErrorSleep,
                         Contract(address=JEDISWAP_ADDR, provider=account, abi=ABI_JEDISWAP),
                         'JediSwap',
    ...
    def swap(self, args: list) -> (bool, int):
        ...
        get_min_amount_out_data = self.contract.functions["get_amounts_out"].prepare(amount_wei, path)
        ...
        calls = [
            ...
            self.contract.functions["swap_exact_tokens_for_tokens"].prepare(amount_wei, min_amount_out, path, self.account.address, deadline)
        ]
